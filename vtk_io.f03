...
subroutine vtk_sig_write ( output_unit, title, node_nums, element_nums, &
  element_order, node, element, U_dt, sig )
...
!
   ...
   type(stress_type),      intent(in) :: sig
...
!
  ...
  write ( output_unit, '(a)' ) 'SCALARS displacement double'
  write ( output_unit, '(a)' ) 'LOOKUP_TABLE default'
  do node_id = 1, node_nums
    write ( output_unit, '(f10.6)' ) U_dt%values(node_id,2) 
  end do
  write ( output_unit, '(a)' ) 'VECTORS stress double'
  do node_id = 1, node_nums
    write ( output_unit, '(3(2x,g14.6))' ) sig%values(node_id,1:3)
  end do

  return
end subroutine vtk_sig_write
end module vtk_io
